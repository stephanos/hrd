...
var _ = Describe("Query", func() {
	...
	Context("multiple entities", func() {

		It("should query no entities", func() {
			query.Filter = append(query.Filter, types.Filter{Filter: "html =", Value: "nonsense"})
			keys, _, err := runQuery(&entities, true)

			Check(err, IsNil)
			Check(keys, HasLen, 0)
			Check(entities, IsEmpty)
		})
		...
		It("should query projection", func() {
			...
			Check(keys, HasLen, 4)
			Check(entities, HasLen, 4)
			Check(entities[0].Text, IsZero)
			Check(entities[0].Num, Not(IsZero))
		})
